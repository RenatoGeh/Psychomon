...
começamos, já que o server e o cliente precisavam daquilo para funcionar.
...
um xsd, que facilitou bastante a encontrar erros no projeto mais para frente, quando começamos
a criar o server e o cliente.

   Porém, tivemos uma grande dificuldade de usar essa biblioteca no Windows, tentando até
mesmo compilar o seu código fonte, sem sucesso. Resolvemos então desenvolver mesmo no
Linux, onde está biblioteca já vem instalada por padrão. Tirando essa dificuldade inicial,
o resto de XML's não foi muito trabalhoso, apenas esforço braçal.

   Depois de pronta essa parte, que criava XML's a partir de Pokemons e Pokemons a partir
de XML's, começamos a criação da interface server/cliente. Para o server usamos a biblioteca
\emph{flask}, que tornou bem fácil a criação de um server funcional, o fluxo da batalha foi meio
diferent do normal, então tivemos que adaptar o resto do programa para isso. Para o cliente
usamos \emph{requests}, e o processo foi bem parecido com a batalha normal.

   A interação entre os dois demorou um pouco para ficar boa, tivemos algumas dificuldades
em mandar e receber dados, mas com um pouco de tempo resolvemos isso. A primeira batalha
...
menos que sejam usadas \emph{gambiarras}) e muito menos da parte do cliente. Além disso, o
...
o usuário se dava em inglês e outras em português. Escolhemos mudar tudo para XXXX.
